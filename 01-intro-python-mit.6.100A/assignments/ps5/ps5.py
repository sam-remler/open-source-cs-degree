...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        ...
        pixel_vector = numpy.array([[r], [g], [b]])
        new_pixel = numpy.matmul(matrix, pixel_vector).flatten()
        # Clamp values between 0 and 255
        new_pixel = tuple(int(min(max(round(val), 0), 255)) for val in new_pixel)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if isinstance(pixel, int):  # BW image
        return pixel & mask
    elif isinstance(pixel, tuple):  # RGB image
        return tuple((channel & mask) for channel in pixel)
...
def reveal_bw_image(filename):
    ...
    new_pixels = [(extract_end_bits(1, p) * 255) for p in pixels]  # Amplify to 0 or 255
    return pix_to_img(new_pixels, (width, height), 'L')
...
def reveal_color_image(filename):
    ...
    for pixel in pixels:
        ...
        # Amplify to 0–255 scale
        new_pixel = (r << 5, g << 5, b << 5)
        new_pixels.append(new_pixel)
    return pix_to_img(new_pixels, (width, height), 'RGB')
