...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        ...
        message = encrypted_message.decrypt_message(pad)
        
        words = message.get_text().split(" ")
        word_count = len([word for word in words if is_word(load_words("words.txt"), word)])
...
def decode_story():
    ...
    pads = get_story_pads()
    best_guess = decrypt_message_try_pads(story_string, pads)

    return best_guess.get_text()
